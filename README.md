# ⚡ Transformerless Power Supply PCB Design using KiCad

A **Transformerless Power Supply PCB** designed using **KiCad 10**. This project converts AC mains voltage into a regulated **5V DC output** using a capacitor-dropper circuit, bridge rectifier, filtering capacitors, Zener diode protection, and an LM7805 voltage regulator.

> ⚠️ **Warning:** This project is directly connected to AC mains voltage. It is intended only for educational purposes. Handle with proper electrical safety precautions.

---

## 📖 Project Overview

This PCB provides a compact and low-cost regulated **5V DC power supply** without using a bulky transformer.

### Features

- Capacitor Dropper Circuit
- Bridge Rectifier (4 × 1N4007)
- Filter Capacitors
- Zener Diode Protection
- LM7805 Voltage Regulator
- LED Power Indicator
- Through-Hole PCB Design

---

## 🛠 Software Used

- KiCad 10
- Schematic Editor
- PCB Editor
- 3D Viewer

---

## 🔩 Components Used

| Component | Value |
|-----------|-------|
| X2 Capacitor | 225K / 2.2µF |
| Capacitor | 0.1µF |
| Capacitor | 1000µF |
| Capacitor | 470µF |
| Diodes | 4 × 1N4007 |
| Zener Diodes | 2 |
| LM7805 | TO-220 |
| LED | 1 |
| Resistors | 1MΩ, 20kΩ ×2, 2.2kΩ |
| Screw Terminals | 2 |

---

## 📂 Repository Structure

```text
Transformerless-Power-Supply-KiCad/
├── Gerber/
├── Images/
├── Transformerless_powersupply.kicad_pcb
├── Transformerless_powersupply.kicad_pro
├── Transformerless_powersupply.kicad_sch
├── README.md
└── .gitignore
```

---

## 📷 Project Images

### Schematic

![Schematic](Images/Schematic_Layout.png)

### PCB Layout

![PCB Layout](Images/PCB_Layout.png)

### 3D Front View

![3D Front View](Images/3D_PCB_Frontview.png)

### 3D Back View

![3D Back View](Images/3D_PCB_Backview.png)

### Footprints

![Footprints](Images/Footprints.png)

---

## 📦 Gerber Files

The **Gerber** folder contains all files required for PCB fabrication:

- Copper Layers
- Solder Mask
- Silkscreen
- Edge Cuts
- Drill Files
- Gerber Job File

---

## ⚠️ Safety

This is a **transformerless power supply**, directly connected to AC mains.

- Use only for educational purposes.
- Do not touch the circuit while powered.
- Always follow electrical safety precautions.

---

## 👨‍💻 Author

**Vunnamatla Venkata Sai Ravi Karthik**

B.Tech – Electrical & Electronics Engineering (EEE)

GitHub: https://github.com/imvrk45

---

## ⭐ Support

If you found this project helpful, please consider giving this repository a **Star ⭐**.